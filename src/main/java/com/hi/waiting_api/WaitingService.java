...
public class WaitingService {
    ...
    public String registerWaiting(String name, String phone) {
        ...
        // 1. [INFO] : 일반적인 비즈니스 흐름(요청 수신)
        log.info("[웨이팅 요청] 이름: {}, 전화번호: {}", name, maskedPhone);
        ...
        // 3. [WARN] 시스템 경고 (대기열 마감 임박) => 50, 100, 1000?
        if (currentWaitingCount >= 2) {
            log.warn("[웨이팅 경고] 대기열 마감 임박! 현재 대기 인원: {}", currentWaitingCount);
        }
